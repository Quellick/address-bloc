 ...
 class MenuController
   ...
   def search_entries
     ...
     system "clear"
     # #11 we check if search returned a match. This expression evaluates to false if  search returns nil since nil evaluates to false in Ruby. 
   ...
   def read_csv
     ...
     # #3 we import the specified file with import_from_csv on address_book. We then clear the screen and print the number of entries that were read from the file. 
     # #  All of these commands are wrapped in a begin/rescue block. begin will protect the program from crashing if an exception is thrown.
     begin
   ...
   def search_submenu(entry)
     ...
     # #14 we use a case statement and take a specific action based on user input. If the user input is d we call delete_entry and after it returns we call main_menu. If the input is e we call edit_entry. m will return the user to the main menu. 
     # #   If the input does not match anything (see the else statement) then we clear the screen and ask for their input again by calling search_submenu.
     case selection
       when "d"
         system "clear"
         delete_entry(entry)
